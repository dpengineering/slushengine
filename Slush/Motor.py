...
class Motor(sBoard):
    ...
    '''
    takes a single parameter, motorNumber, and sets the chip select based on that input
    input ranges from 0 to 6
    '''
    ...
    def initPeripherals(self):
        ...
        if (self.getParam(LReg.CONFIG) == 0x2e88):
            ...
            # prints if the SPI is not working
            print("SPI not working; communication issues; check SPI configuration and cables")

        '''
        based on board type init driver accordingly
        gets set in the above if statements
        '''
    ...
    moves the motor with a direction (0/1) and a speed 
     '''
    ...
    takes some action that will stop when pressed
    also takes a direction (0/1) and speed
    '''
    ...
    ''' 
    hard stop the motor
    doesn't account for decel curve
    '''
    def hardStop(self):
